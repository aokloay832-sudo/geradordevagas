...
# ==============================================================================
# CONFIGURAÇÕES PERSONALIZADAS DO TEXTO DE DISPARO
# ==============================================================================
NOME_RECRUTADOR = "Renato Monteiro"
EMPRESA_RECRUTADORA = "EB CORPORATE RH"
EMPRESA_PARCEIRA = "CATHO"
LINK_CADASTRO = "https://ebcrecursoshumanos.com"
# ==============================================================================
...
def bot_send_message(chat_id, text, reply_markup=None, parse_mode="Markdown"):
    ...
    # Tentativa 1: Enviar com o parse_mode solicitado (ex: Markdown)
    if parse_mode:
        ...
        try:
            ...
            logger.warning(f"⚠️ Falha no envio com {parse_mode}. Erro: {e}. Tentando modo sem formatação...")

    # Tentativa 2 (Fallback): Se o Markdown falhar por erro de caractere, envia texto puro
    payload.pop("parse_mode", None)
    try:
        response = requests.post(url, json=payload, timeout=10)
        response.raise_for_status()
        logger.info("✅ Mensagem enviada com sucesso em modo texto puro (fallback).")
    except requests.exceptions.RequestException as e:
        logger.error(f"❌ Erro crítico ao enviar mensagem para o Telegram: {e}")
...
Meu nome é {NOME_RECRUTADOR} e falo em nome da *{EMPRESA_RECRUTADORA}*. Somos parceiros da empresa *{EMPRESA_PARCEIRA}* e selecionamos o seu currículo para uma *vaga* alinhada ao seu perfil.
    ...
    if "message" in update:
        ...
        # FLUXO 2: GERAR TEXTO PRA DISPARO
            ...
            if state.get("step") == "aguardando_info_candidato":
                ...
                # Envia sem parse_mode para garantir que não haja erros de tags
